    ...
    func sendCommand(
        ...
        if prefix == "01" || prefix == "06" || prefix == "09" {
            ...
            if response.count > 18 {
                ...
                let ffLength =
                    ff.replacingOccurrences(of: " ", with: "").count / 2

                Totallength += ffLength

                var cf = Array(chunks.dropFirst())
                Totallength +=
                    cf.joined().replacingOccurrences(of: " ", with: "").count
            ...
            String(
                self[
                    self.index(
                        ...
                        self.startIndex,
                        offsetBy: min($0 + chunkSize, self.count)
                    )
                ]
            )
        }
