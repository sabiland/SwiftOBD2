...
struct MonitorDecoder: Decoder {
    func decode(data: Data, unit: MeasurementUnit) -> Result<
        DecodeResult, DecodeError
    > {
        guard data.count >= 9 else { return .failure(.invalidData) }
    ...
    func parse_monitor_test(_ data: Data) -> MonitorTest? {
        ...
        guard let uas = uasIDS[cid] else {
            print(
                "Encountered Unknown Units and Scaling ID: ",
...
struct FuelRateDecoder: Decoder {
    func decode(data: Data, unit: MeasurementUnit) -> Result<
        DecodeResult, DecodeError
    > {
        guard !data.isEmpty else { return .failure(.invalidData) }
...
struct FuelTypeDecoder: Decoder {
    func decode(data: Data, unit: MeasurementUnit) -> Result<
        DecodeResult, DecodeError
    > {
        guard let i = data.first else { return .failure(.invalidData) }
        var value: String?
...
struct MaxMafDecoder: Decoder {
    func decode(data: Data, unit: MeasurementUnit) -> Result<
        DecodeResult, DecodeError
    > {
        guard let byte = data.first else { return .failure(.invalidData) }
        let value = byte * 10
        return .success(
            (.measurementResult(
                MeasurementResult(
...
struct O2SensorsDecoder: Decoder {
    func decode(data: Data, unit: MeasurementUnit) -> Result<
        DecodeResult, DecodeError
    > {
        guard data.count >= 1 else { return .failure(.invalidData) }
...
struct TimingAdvanceDecoder: Decoder {
    ...
    > {
        guard let byte = data.first else { return .failure(.invalidData) }
        let value = Double(byte) / 2.0 - 64.0
        return .success(
            .measurementResult(
...
struct FuelStatusDecoder: Decoder {
    ...
    > {
            ...
            let index = lowBits.firstIndex(of: 1)
        {
            ...
            print(
                "Invalid response for fuel status (multiple/no bits set in low bits)"
            )
        }
...
func parseDTC(_ data: Data) -> TroubleCode? {
    ...
    return TroubleCode(
        code: dtc,
        description: codes[dtc] ?? "No description available."
    )
}
